...
def _uq_to_dict(field_name: str, uq: Optional[UncertainQuantity]) -> dict:
    ...
    if uq is None:
        return {field_name: None, f"{field_name}_lower": None, f"{field_name}_upper": None}
    return {
        field_name:          uq.value,
...
class GWEvent(BaseModel):
    ...
    def model_validate(cls, obj, **kwargs):
        ...
        obj['pastro_ready'] = 'PASTRO_READY' in labels or 'PASTRO_READY' in preferred.get('labels', [])
    ...
    def from_gwosc(cls, gw_name: str) -> "GWEvent":
        ...
        preferred_pe = next(
            (p for p in latest["parameters"].values() if p.get("is_preferred")),
            None
        )
    ...
    def enrich_from_gwosc(self) -> "GWEvent":
        ...
        try:
            ...
            pe_fields = [
                'catalog', 'gps', 'far', 'p_astro', 'network_snr',
                'mass_1', 'mass_2', 'chirp_mass', 'total_mass', 'final_mass',
                'luminosity_distance', 'redshift', 'chi_eff',
                'preferred_waveform', 'posterior_url', 'strain_files',
            ]
    ...
    def get_90_roi_rect(self):
        ...
        return {
            ...
            'dec_min': dec.min(),
            'dec_max': dec.max(),
            'area_deg2': len(inside_90) * ah.nside_to_pixel_area(nside).to(u.deg**2).value
        }
    ...
    def plot_event(self, figPath: Path | str | None = None, circle_roi: bool = False, rect_roi : bool = False):
        skymap, meta = self.load_skymap()
...
def _build_gwevent(superevent_id: str, client=None, classification: bool = True) -> GWEvent:
    if client is None:
        client = GraceDb()
...
def _build_gwevent_from_gw_name(gw_name: str, client=None, classification: bool = True) -> GWEvent:
    if client is None:
        client = GraceDb()
    ...
    if classification:
        _fetch_classification(sev, client)
    pe_fields = [
        'gw_id', 'catalog', 'gps', 'far', 'p_astro', 'network_snr',
        'mass_1', 'mass_2', 'chirp_mass', 'total_mass', 'final_mass',
        'luminosity_distance', 'redshift', 'chi_eff',
        'preferred_waveform', 'posterior_url', 'strain_files',
    ]
